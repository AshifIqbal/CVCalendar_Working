...
class CVCalendarView: UIView {
    ...
    func reloadControllerData()
    {
        return
            ...
            // the following call causes error on later context. Avoid using this function call before fixing the issues.
            
//        self.contentController.presentedMonthView.reloadView() 
    }
...
extension CVCalendarView {
    ...
    func reloadCalendar(mode: CalendarMode) {
        
        if let selectedDate = coordinator.selectedDayView?.date.convertedDate() {
            calendarMode = mode
